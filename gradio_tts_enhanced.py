...
AUD_PROC = AudioPreprocessor(
    target_sr=48_000, target_lufs=-16.0, peak_ceiling_db=-2.0, use_ffmpeg=False
)
ASR_QA = ASRVerifier(model_name="base", use_faster_whisper=True, device=DEVICE)
...
def _tts_chunks(
    model: ChatterboxTTS, text: str, ref_path: str | None, **params
    ...
    sr = model.sr
    silence = np.zeros(int(0.2 * sr))  # 200 ms between chunks
    ...
    merged_wav = np.concatenate(wavs)
    # cleaned, new_sr = AUD_PROC.preprocess_array(merged, sr)  # loudness + peak
    return merged_wav, sr
    # return cleaned, new_sr
...
def generate(
    ...
    qa_enable: bool,
) -> tuple[tuple[int, np.ndarray], str, float]:
    """Main callback for the Gradio button."""
    if model is None:
        model = load_model()
    ...
    # Save to a temp file so ASR can read it (faster‑whisper wants a path)
    tmp_wav_path = Path(tempfile.gettempdir()) / f"cbx_{uuid.uuid4().hex}.wav"
    logger.debug(f"temp wave audio path: {tmp_wav_path}\n")
    sf.write(tmp_wav_path, wav, sr)

    transcript, score = "", 0.0
    if qa_enable:
        res = ASR_QA.verify(str(tmp_wav_path), text)
        transcript, score = res.transcription, res.score

    return (sr, wav), transcript, score
...
The crownless again shall be king.""",
            ...
            with gr.Accordion("More options", open=False):
                ...
                rep_penalty = gr.Slider(
                    1.0, 2.0, step=0.1, label="Repetition penalty", value=1.2
                )

            qa_enable = gr.Checkbox(label="Verify output with ASR", value=False)
        ...
        with gr.Column(scale=1):
            ...
            transcript_out = gr.Textbox(
                label="ASR transcript (auto-evaluated)", lines=4
            )
    ...
    run_btn.click(
        ...
        outputs=[audio_out, transcript_out, score_out],
    )
